...
class DeepTIMe(nn.Module):
    def __init__(self, datetime_feats: int, layer_size: int, inr_layers: int, n_fourier_feats: int, scales: float, device: str):
        super().__init__()
        ...
        self.adaptive_weights = RidgeRegressor(device = device)
    ...
    def forward(self, x: Tensor, x_time: Tensor, y_time: Tensor) -> Tensor:
        ...
        coords = self.get_coords(lookback_len, tgt_horizon_len).to( self.device )
